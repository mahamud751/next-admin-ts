...
const EmployeeDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (data.overview) {
      ...
      const attendancePieArr = Object.keys(attendancePieObj).map((item) => ({
        label: item,
  ...
  const handleFilter = (e) => {
    ...
    if (e === "Today") {
      ...
      setFromDate(DateTime.now().plus({ days: 1 }).toFormat("yyyy-MM-dd"));
      ...
      setToDate(
        DateTime.now().minus({ weeks: 1 }).endOf("week").toFormat("yyyy-MM-dd")
      );
        ...
        <Grid container spacing={3}>
              ...
              <CardContent>
                  ...
                  <Link
                    to={{
                      ...
                      search: `filter=${JSON.stringify({
                        _attendance_status: "",
                        _attendance_status_type: "leave,holiday",
                        _attendance_date: fromDate,
                        _attendance_date_end: toDate,
                        _department: department,
                        _include_child_department: includeChildDepartment,
                ...
                <EmployeeList
                  filter={{
                    ...
                    shift,
                    department,
                    include_child_department: includeChildDepartment,
                    employee,
                    status: "",
          ...
          <Grid item xs={12} sm={6}>
            <Card>
              <CardContent>
                <Box display="flex" justifyContent="space-between" mb={1}>
                  <Typography variant="h6">Comming</Typography>
                  <Link
                    to={{
                      pathname: "/v1/employeeAttendance",
                      search: `filter=${JSON.stringify({
                ...
                <Box display="flex" justifyContent="space-between" mb={1}>
                  ...
                  <Link
                    to={{
                      pathname: "/v1/employeeAttendance",
                      search: `filter=${JSON.stringify({
                        _attendance_status: "present,delayed",
                        _attendance_status_type: "",
                        _attendance_date: fromDate,
                        _attendance_date_end: toDate,
                        _department: department,
                ...
                <Box display="flex" justifyContent="space-between" mb={1}>
                  ...
                  <Link
                    to={{
                      ...
                      search: `filter=${JSON.stringify({
                        _attendance_status: "",
                        _attendance_status_type: "active",
                        _attendance_date: fromDate,
                        _attendance_date_end: toDate,
                        _department: department,
                        _include_child_department: includeChildDepartment,
              ...
              <CardContent>
                <Box display="flex" justifyContent="space-between" mb={1}>
                  ...
                  {isLoadingActive ? (
                    ...
                    <LoaderOrButton
                      label="Load"
                      onClick={() => setisLoadingActive(true)}
                    />
                  )}
                </Box>
                {isLoadingActive && (
              ...
              <CardContent>
                <Box display="flex" justifyContent="space-between" mb={1}>
                  ...
                  {isLoadingAbsent ? (
                    ...
                    <LoaderOrButton
                      label="Load"
                      onClick={() => setisLoadingAbsent(true)}
                    />
                  )}
                </Box>
                {isLoadingAbsent && (
              ...
              <CardContent>
                ...
                {!!data?.overview && (
                  <LineChart
                    ...
                    chartData={data?.overview ? data.overview : []}
