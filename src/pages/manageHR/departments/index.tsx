      ...
      {!!departments?.length && (
        <TableContainer>
          <Table className={classes.table} size="small">
            ...
            {departments
              .filter((item) => item?.t_parent_id === 0 || item?.isOpen)
              .map((department) => {
                const {
                  t_id,
                  t_parent_id,
                ...
                return (
                  <TableRow
                    ...
                    onDragLeave={() => handleDragLeave(t_id)}
                    onDrop={(e) => handleDrop(e, t_id)}
                    draggable
                    style={{
                      backgroundColor: dragLeaveId === t_id ? "#3ed9c9" : "",
                    }}
                  >
                    <TableCell>
                      ...
                      >
                        {isExpand ? (
                          <ArrowDropDownIcon
                            ...
                            onClick={() =>
                              handleExpandCollapse("collapse", t_id)
                            }
                          />
                        ) : (
                          <ArrowRightIcon
                            ...
                            onClick={() => handleExpandCollapse("expand", t_id)}
                          />
                        )}
                        {capitalizeFirstLetterOfEachWord(t_title)}
                      </Box>
                    </TableCell>
                    ...
                    <TableCell>
                      <AroggaButton
                        ...
                        onClick={() => {
                          setAction("edit");
                          setSingleDepartmentData(department);
                          setIsCreateDialogOpen(true);
                        }}
                      />
                    </TableCell>
