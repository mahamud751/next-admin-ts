...
const DesignationsPage = () => {
  ...
  } = useRequest("/v1/rank?_page=1&_perPage=5000", {}, { isPreFetching: true });
      ...
      {!!designations?.length && (
        <TableContainer>
          <Table className={classes.table} size="small">
            ...
            {designations
              .filter((item) => item?.r_parent === 0 || item?.isOpen)
              .map((designation) => {
                const {
                  r_id,
                  r_parent,
                ...
                return (
                  <TableRow
                    ...
                    onDragLeave={() => handleDragLeave(r_id)}
                    onDrop={(e) => handleDrop(e, r_id)}
                    draggable
                    style={{
                      backgroundColor: dragLeaveId === r_id ? "#3ed9c9" : "",
                    }}
                  >
                    <TableCell>
                    ...
                    <TableCell>
                      ...
                      >
                        {isExpand ? (
                          <ArrowDropDownIcon
                            style={{
                              display: hasChildren ? "" : "none",
                              cursor: "pointer",
                            }}
                            onClick={() =>
                              handleExpandCollapse("collapse", r_id)
                            }
                          />
                        ) : (
                          <ArrowRightIcon
                            ...
                            onClick={() => handleExpandCollapse("expand", r_id)}
                          />
                        )}
                        {capitalizeFirstLetterOfEachWord(r_title)}
                      </Box>
                    </TableCell>
                    <TableCell>{r_sick_leaves}</TableCell>
                    <TableCell>{r_casual_leaves}</TableCell>
                    <TableCell>{r_annual_leaves}</TableCell>
                    <TableCell>{r_compensatory_leaves}</TableCell>
                    <TableCell>{r_maternity_leaves}</TableCell>
                    <TableCell className={classes.whitespaceNowrap}>
                      <AroggaButton
                        ...
                        onClick={() => {
                          setAction("edit");
                          setSingleDesignationData(designation);
                          setIsCreateDialogOpen(true);
                        }}
                      />
                      <AroggaButton
