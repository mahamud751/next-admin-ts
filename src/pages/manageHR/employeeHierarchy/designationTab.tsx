...
const DesignationTab = () => {
  ...
  const { data: ranks, isLoading } = useRequest(
    "/admin/v1/employeeHierarchy/rank",
    {},
            ...
            label={
              <div className={classes.nodeStyle}>
                ...
                <small
                  ...
                  onClick={() =>
                    handleClick(getTotalEmployees(row, "selfEmployee"))
                  }
                >
                  {" "}
                  Emp: {countTotalEmployees(row, "selfEmployee")}{" "}
                </small>
                <br />
                <small
                  style={{
                  ...
                  onClick={() =>
                    handleClick(getTotalEmployees(row, "childEmployee"))
                  }
                >
                  {" "}
                  Child Emp: {countTotalEmployees(row, "childEmployee")}{" "}
                </small>
                <br />
                <small
                  style={{
                  ...
                  onClick={() =>
                    handleClick(getTotalEmployees(row, "totalEmployee"))
                  }
                >
                  {" "}
                  Total Emp: {countTotalEmployees(row, "totalEmployee")}{" "}
                </small>
              </div>
            }
          >
      ...
      {isLoading ? (
        ...
        <>
          <Tree
            ...
            label={<div className={classes.nodeStyle}>Designations</div>}
          >
            {genderTree(ranks)}
          </Tree>
          <Dialog
                      ...
                      {employees.length
                        ? employees.map((item, index) => (
                            <TableRow>
                              ...
                              <TableCell>{item.e_name}</TableCell>
                              <TableCell>{item.e_mobile}</TableCell>
                            </TableRow>
                          ))
                        : null}
                    </TableBody>
