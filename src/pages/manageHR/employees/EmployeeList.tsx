...
const EmployeeList: FC<ListProps> = ({ ...rest }) => {
  useDocumentTitle("Arogga | Employee List");
  const { permissions } = usePermissions();
  const classes = useAroggaStyles();
  const redirect = useRedirect();
  const navigateFromList = useNavigateFromList("employeeView", "employeeEdit");
  const location = useLocation();
  const { _shiftType } = toFilterObj(location.search);
  ...
  const exporter = (records, fetchRelatedRecords) => {
    fetchRelatedRecords(records, "e_rank_id", "v1/rank")
      .then((ranks) => {
        const data = records?.map(({ id, e_rank_id, ...restRecord }) => ({
          ...restRecord,
          designation: ranks?.[e_rank_id]?.r_title,
        }));

        return jsonExport(data, {}, (_, csv) => downloadCSV(csv, "employees"));
      })
  ...
  return (
    ...
    >
      ...
      >
        ...
        <TextField source="e_compensatory_leaves" label="Compensatory Leaves" />
        <TextField source="e_maternity_leaves" label="Maternity Leaves" />
        <FunctionField
        ...
        />
        <AroggaDateField source="e_date_of_joining" label="Joining Date" />
        <AroggaDateField
          source="e_confirmation_date"
          label="Confirmation Date"
        />
        <AroggaDateField source="e_date_of_leaving" label="Leaving Date" />
        <AroggaDateField source="e_date_of_release" label="Releasing Date" />
        <BooleanField
          ...
          render={({ e_user_id, e_rank_id }: Record) => (
            <Box display="flex" gap={5}>
              <Button
                label="User"
                variant="contained"
                onClick={() => redirect(`/v1/users/${e_user_id}/show`)}
              />
              <Button
                label="Rank Permission"
                variant="contained"
                className={classes.whitespaceNowrap}
                onClick={() => redirect(`/designation-permission/${e_rank_id}`)}
              />
            </Box>
          )}
        />
