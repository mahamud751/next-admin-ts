...
import LineChartCustomTooltip from "@/components/dashboard/LineChartCustomTooltip";
import { capitalizeFirstLetterOfEachWord } from "@/utils/helpers";

type LineChartComponentProps = {
  from?: "agent" | "delivery" | "checking" | "packing";
  hideState: object;
...
const LineChartComponent: FC<LineChartComponentProps> = ({
  ...
  chartData?.map((item) => {
    Object.keys(item).map((key) => {
      if (!key.includes("_order_count") && key !== "date") {
        userSum[key] = userSum[key] ? userSum[key] + item[key] : item[key];
      }
      return key;
    });
  ...
  const isFrom = ["agent", "delivery", "checking", "packing"].includes(from);
          ...
          <Legend
            ...
            formatter={(value) => (
              <span className="recharts-legend-item-text">
                {value.split("_")[1] || "Total"} ({userSum[value]})
              </span>
            )}
            onClick={(e) => {
        ...
        {!!shortByUser?.length &&
          shortByUser.map((item, index) => (
            <Line
              key={index}
              type="monotone"
              dataKey={capitalizeFirstLetterOfEachWord(item)}
              xlinkShow="new"
